...
func NewPeerHandler(coord MessageHandlerCoordinator, stream ChatStream, initiatedStream bool, nextHandler MessageHandler) (MessageHandler, error) {
	...
	if dur, err := time.ParseDuration(viper.GetString("peer.sync.state.snapshot.writeTimeout")); err != nil || dur == 0 {
...
func (d *Handler) beforeSyncStateSnapshot(e *fsm.Event) {
	...
	if d.snapshotRequestHandler.shouldHandle(syncStateSnapshot.Request.CorrelationId) {
		...
		if lastIgnored == nil || *lastIgnored != syncStateSnapshot.Request.CorrelationId {
			peerLogger.Warningf("Ignoring SyncStateSnapshot message with correlationId = %d, sequence = %d, as current correlationId = %d, future messages for this id will be suppressed", syncStateSnapshot.Request.CorrelationId, syncStateSnapshot.Sequence, d.snapshotRequestHandler.correlationID)
			lastIgnored = &syncStateSnapshot.Request.CorrelationId
			//Ignore the message, does not match the current correlationId
		}
	}
}
