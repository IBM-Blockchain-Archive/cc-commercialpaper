...
func (sts *coordinatorImpl) SyncToTarget(blockNumber uint64, blockHash []byte, peerIDs []*protos.PeerID) (error, bool) {
	logger.Infof("%v attempting to sync to target %x for block number %d with peers %v", sts.id, blockHash, blockNumber, peerIDs)
	bhr := &blockHashReply{
		syncMark: syncMark{
...
func (sts *coordinatorImpl) syncBlockchainToCheckpoint(blockSyncReq *blockSyncReq) {
	...
	logger.Debugf("%v is processing a blockSyncReq to block %d through %d", sts.id, blockSyncReq.blockNumber, blockSyncReq.reportOnBlock)
	...
	if blockSyncReq.blockNumber+1 < blockchainSize {
		...
		if blockCursor < blockSyncReq.blockNumber {
			logger.Debugf("%v skipped remote syncing of block %d through %d because it they were already present and valid", sts.id, blockSyncReq.blockNumber, blockCursor+1)
		}
		var err error
